...
1. Fake objects with `HelidonTest` - Allows a CDI bean to be replaced with a fake implementation using HelidonTest 
   library. For this approach, we will be using various components of HelidonTest:
   ...
   * `WebTarget` - an injectable object that can be used as the http interface to invoke the server endpoint started 
   by `@HelidonTest`
2. Mocking/stubbing using `Mockito` - Allows a class or interface to be mocked using java reflection via the Mockito
   framework. Specific to this tutorial, we will only use a subset of Mockito components:
